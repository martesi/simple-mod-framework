 ...
 */
export function validateModFolder(modFolder: string, manifest: DiskManifest): { valid: boolean; error?: string } {
  for (const field of ["id", "name", "description", "authors", "version", "frameworkVersion"] as const) {
    if (manifest[field] === undefined || manifest[field] === null) {
      return { valid: false, error: `Manifest is missing required field "${field}"` }
    }
  }

  const allContentFolders = [...(manifest.contentFolders || []), ...(manifest.options || []).flatMap((a) => a.contentFolders || [])]

  for (const contentFolder of allContentFolders) {
    const full = resolve(modFolder, contentFolder)
    if (!existsSync(full)) {
      return { valid: false, error: `Invalid content folder "${contentFolder}" due to nonexistent path` }
    }
    ...
    for (const chunkFolder of chunkFolders) {
      if (!chunkFolder.match(/chunk([0-9]*)/)) {
        return { valid: false, error: `Invalid chunk folder "${chunkFolder}" in "${contentFolder}"` }
      }
    }
  }

  const allBlobsFolders = [...(manifest.blobsFolders || []), ...(manifest.options || []).flatMap((a) => a.blobsFolders || [])]

  for (const blobsFolder of allBlobsFolders) {
    const full = resolve(modFolder, blobsFolder)
    if (!existsSync(full)) {
      return { valid: false, error: `Invalid blobs folder "${blobsFolder}" due to nonexistent path` }
    }
  ...
  for (const [group, { members, enabledByDefault }] of Object.entries(groups)) {
    ...
    if (enabledByDefault > 1) return { valid: false, error: `Option group "${group}" has more than one member enabled by default` }
  }
