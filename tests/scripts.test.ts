...
import { errorStack, formatScriptError, requireTask, runBestEffort, scriptError } from "../scripts/lib/effects"
...
describe("script effects", () => {
	...
	test("runs every best-effort task and remains successful", async () => {
		...
		await requireTask(runBestEffort([
			{ label: "one", warning: "fix one", operation: TE.rightIO(() => { ran.push("one") }) },
			{ label: "two", warning: "fix two", operation: TE.left(scriptError("download", "offline")) }
