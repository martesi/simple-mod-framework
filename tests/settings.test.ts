...
const dataRoot = join(
  tmpdir(),
  `smf-settings-test-${Date.now()}-${Math.random().toString(36).slice(2)}`
)
...
test('game-root defaults resolve consistently for root and Retail paths', () => {
  ...
  expect(resolveTempDir(paths, { ...settings, gamePath: join(gameRoot, 'Retail') })).toBe(
    join(gameRoot, '.smf', 'tmp')
  )
  expect(resolveModsDir(paths, { ...settings, gamePath: join(gameRoot, 'Retail') })).toBe(
    join(gameRoot, '.smf', 'mods')
  )
})
