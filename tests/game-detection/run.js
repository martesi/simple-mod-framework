...
try {
  ...
  check(
    'a PC install without a reliable hint remains unresolved instead of defaulting to Steam',
    () => {
      const result = gameDetect.deriveGamePathInfoUncached(gameRoot, {
        dataRoot,
        toolsRoot: path.join(scratchDir, 'tools'),
      })
  ...
  check('storefront DLL markers provide a best-effort default', () => {
    ...
    assert.equal(
      gameDetect.deriveGamePathInfoUncached(gameRoot, {
        dataRoot,
        toolsRoot: path.join(scratchDir, 'tools'),
      }).platform,
      'steam'
  ...
  check('an explicit storefront choice is persisted and reused', () => {
    ...
    const first = gameDetect.deriveGamePathInfo(
      gameRoot,
      { dataRoot, toolsRoot: path.join(scratchDir, 'tools') },
      'epic'
    )
    ...
    const cached = gameDetect.deriveGamePathInfo(gameRoot, {
      dataRoot,
      toolsRoot: path.join(scratchDir, 'tools'),
    })
    ...
    const overridden = gameDetect.deriveGamePathInfo(
      gameRoot,
      { dataRoot, toolsRoot: path.join(scratchDir, 'tools') },
      'steam'
    )
  ...
  check('a Microsoft Store root is normalized to Retail before layout detection', () => {
    ...
    fs.writeFileSync(
      path.join(microsoftRoot, 'MicrosoftGame.Config'),
      'an updated Microsoft config'
    )
    fs.writeFileSync(path.join(microsoftRetail, 'thumbs.dat'), 'thumbs')

    const result = gameDetect.deriveGamePathInfoUncached(microsoftRoot, {
      dataRoot,
      toolsRoot: path.join(scratchDir, 'tools'),
    })
    assert.equal(result.ok, true)
