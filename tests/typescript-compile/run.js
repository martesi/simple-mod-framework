...
function check(name, fn) {
	try {
		...
		console.error(`    ${e && e.stack ? e.stack.replace(/\n/g, "\n    ") : e}`)
	}
}
...
async function main() {
	...
	let entryPath
	await checkAsync("compiles TS (types/interfaces stripped, enum + cross-file import/export intact) and produces runnable output", async () => {
		...
		assert.strictEqual(result, "HELLO, ROGER (X3)!", "enum/import/export/type-stripping produced wrong runtime output")
	})

	check("output was actually produced by esbuild (not some other transpiler silently matching behavior)", () => {
		...
		assert.ok(/__toCommonJS/.test(code) && /__export/.test(code), "expected esbuild's characteristic cjs interop helpers in the output")
	})

	await checkAsync("cache hit: identical inputs return the same path without recompiling", async () => {
		const fileNames = [path.join(modDir, "main.ts"), path.join(modDir, "helper.ts")]
		const start = Date.now()
		const entryPath2 = await compile(fileNames, { target: "es2019" }, modDir)
		const elapsedMs = Date.now() - start
		assert.strictEqual(entryPath2, entryPath, "cache hit returned a different path than the original compile")
		...
		assert.ok(elapsedMs < 100, `expected a cache hit to be near-instant, took ${elapsedMs}ms - esbuild may be getting re-invoked unnecessarily`)
	})

	await checkAsync("changing a source file invalidates the cache (different hash, different entry)", async () => {
		...
		assert.notStrictEqual(pathV1, pathV2, "expected changing source content to produce a different cache entry")
	...
	await checkAsync("rootDir-escape guard still rejects a script outside its mod folder", async () => {
		...
		const outsideDir = fs.mkdtempSync(path.join(os.tmpdir(), "smf-typescript-compile-test-outside-"))
		const outsideFile = path.join(outsideDir, "outside.ts")
		fs.writeFileSync(outsideFile, `export async function analysis() { return "should never run" }\n`)
		try {
			await compile([outsideFile], { target: "es2019" }, modDir)
			assert.fail("expected compile() to throw for a fileName outside rootDir")
		} catch (e) {
			assert.ok(/escapes its mod folder/.test(e.message), `expected the escape-guard error, got: ${e.message}`)
		} finally {
