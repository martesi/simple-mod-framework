...
import {
  beginModBuild,
  finishModBuildFailed,
  finishModBuildReady,
  getModBuild,
  getRpkgHashCacheEntries,
  setRpkgHashCacheEntries,
} from '../db'
...
import { config, FrameworkVersion, logger, paths, rpkgInstance } from './core-singleton'
...
export const execCommand = (command: string) => {
  ...
  child_process.execSync(wrapped, {
...
export const callRPKGFunction = async (command: string) => {
...
export const getRPKGOfHash = async (hash: string): Promise<string> => {
...
export function loadRPKGHashCache() {
  Object.assign(
    RPKGHashCache,
    Object.fromEntries(Object.entries(getRpkgHashCacheEntries()).map((a) => [a[0], [a[1], false]]))
  )
}
 ...
 */
export async function serialiseDeployInstruction(
  deployInstruction: DeployInstruction
): Promise<string> {
  const content = await Promise.all(
    deployInstruction.content.map(async (entry) =>
      entry.source === 'virtual' ? { ...entry, content: await blobToJSON(entry.content) } : entry
    )
  )

  const blobs = await Promise.all(
    deployInstruction.blobs.map(async (entry) =>
      entry.source === 'virtual' ? { ...entry, content: await blobToJSON(entry.content) } : entry
    )
  )
  ...
  for (const [chunk, data] of Object.entries(deployInstruction.rpkgTypes)) {
    rpkgTypes[chunk] =
      data.type === 'base' && data.chunkMeta instanceof Blob
        ? { type: 'base', chunkMeta: await blobToJSON(data.chunkMeta) }
        : data
  }
...
export function deserialiseDeployInstruction(serialised: string): DeployInstruction {
  ...
  const restoreBlob = (value: any) =>
    value && typeof value === 'object' && value[BLOB_MARKER] ? jsonToBlob(value) : value

  parsed.content = parsed.content.map((entry: any) =>
    entry.source === 'virtual' ? { ...entry, content: restoreBlob(entry.content) } : entry
  )
  parsed.blobs = parsed.blobs.map((entry: any) =>
    entry.source === 'virtual' ? { ...entry, content: restoreBlob(entry.content) } : entry
  )
...
export function loadReadyDeployInstruction(modId: string): DeployInstruction | undefined {
  ...
  if (build?.status !== 'ready' || !build.deployInstructionJson) return undefined
...
export default async function analyseMod(mod: string): Promise<DeployInstruction | undefined> {
  ...
  if (!resolved.isFrameworkMod) {
    await logger.warn(
      `"${mod}" is an RPKG-only mod (no manifest.json) - these are staged directly during deploy and have no build to cache.`
    )
    return undefined
  }
  ...
  const manifest: Manifest = ((resolved.manifest as unknown as Manifest) ??
    normalizeManifest(
      json5.parse(fs.readFileSync(path.join(config.modsPath, modFolder, 'manifest.json'), 'utf8'))
  ...
  try {
    ...
    for (const contentFolder of manifest.contentFolders || []) {
      if (
        ...
        fs.existsSync(path.join(config.modsPath, modFolder, contentFolder)) &&
        fs.readdirSync(path.join(config.modsPath, modFolder, contentFolder)).length
      ) {
    ...
    for (const blobsFolder of manifest.blobsFolders || []) {
      if (
        blobsFolder?.length &&
        fs.existsSync(path.join(config.modsPath, modFolder, blobsFolder)) &&
        fs.readdirSync(path.join(config.modsPath, modFolder, blobsFolder)).length
      ) {
    ...
    if (config.modOptions[manifest.id] && manifest.options?.length) {
      ...
      for (const option of manifest.options.filter(
        ...
        if (option.localisationOverrides)
    ...
    for (const contentFolder of [...contentFolders].sort()) {
      for (const chunkFolder of fs
        .readdirSync(path.join(config.modsPath, modFolder, contentFolder))
        .sort()) {
        for (const contentFile of (
          await walk(path.join(config.modsPath, modFolder, contentFolder, chunkFolder))
        )
          ...
          if (path.basename(contentFilePath) === 'localisation.patch.json') {
            const patch = parseLocalisationPatch(
              JSON.parse(fs.readFileSync(contentFilePath, 'utf8')),
              contentFilePath
            )
        ...
        if (
          fs.existsSync(
            path.join(config.modsPath, modFolder, contentFolder, chunkFolder, `${chunkFolder}.meta`)
          )
        ) {
    ...
    for (const blobsFolder of blobsFolders) {
      for (const blobFile of (
        await walk(path.join(config.modsPath, modFolder, blobsFolder))
      ).filter((a) => a.stats.isFile())) {
        const blob = blobFile.path
        const blobPath = blob
          .replace(path.join(config.modsPath, modFolder, blobsFolder), '')
          .slice(1)
          .split(path.sep)
          .join('/')
          .toLowerCase()
        ...
        if (path.extname(blob).startsWith('.jp') || path.extname(blob) === '.png') {
          ...
          blobHash = `00${md5(
            `[assembly:/_pro/online/default/cloudstorage/resources/${blobPath}].pc_${path.extname(blob).slice(1)}`.toLowerCase()
          )
            .slice(2, 16)
            .toUpperCase()}`
        }
    ...
    const deployInstruction: DeployInstruction = {
      ...
      manifestSources: {
        ...
        peacockPlugins: (manifest.peacockPlugins || []).map((a) =>
          path.join(config.modsPath, modFolder, a)
        ),
        scripts,
      },
      content,
    ...
    finishModBuildReady(
      manifest.id,
      FrameworkVersion,
      await serialiseDeployInstruction(deployInstruction)
    )
