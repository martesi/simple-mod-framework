...
function ensureWorkerCore(
  workerConfig: Config,
  coreOptions: ResolvedCoreOptions,
  workerPaths: { dataRoot: string; toolsRoot: string }
) {
...
async function processPatch({
  ...
  if (
    !(await copyFromCache(
      cacheFolder,
      path.join(chunkFolder, await xxhash3(patches[patches.length - 1].path)),
      path.join(paths.dataRoot, assignedTemporaryDirectory)
    ))
  ) {
    ...
    const callRPKGFunction = async (command: string) => {
    ...
    if (!fs.existsSync(path.join(paths.dataRoot, 'staging', chunkFolder, `${tempHash}.TEMP`))) {
      await callRPKGFunction(
        `-extract_from_rpkg "${path.join(config.runtimePath, `${tempRPKG}.rpkg`)}" -filter "${tempHash}" -output_path "${path.join(paths.dataRoot, assignedTemporaryDirectory)}"`
      )
    } else {
      try {
      ...
      await Promise.all([
        fs.copyFile(
          path.join(paths.dataRoot, 'staging', chunkFolder, `${tempHash}.TEMP`),
          path.join(
            paths.dataRoot,
            assignedTemporaryDirectory,
            tempRPKG,
        ...
        fs.copyFile(
          path.join(paths.dataRoot, 'staging', chunkFolder, `${tempHash}.TEMP.meta`),
          path.join(
            paths.dataRoot,
            assignedTemporaryDirectory,
            tempRPKG,
    ...
    if (!fs.existsSync(path.join(paths.dataRoot, 'staging', chunkFolder, `${tbluHash}.TBLU`))) {
      await callRPKGFunction(
        `-extract_from_rpkg "${path.join(config.runtimePath, `${tbluRPKG}.rpkg`)}" -filter "${tbluHash}" -output_path "${path.join(paths.dataRoot, assignedTemporaryDirectory)}"`
      )
    } else {
      try {
      ...
      await Promise.all([
        fs.copyFile(
          path.join(paths.dataRoot, 'staging', chunkFolder, `${tbluHash}.TBLU`),
          path.join(
            paths.dataRoot,
            assignedTemporaryDirectory,
            tbluRPKG,
        ...
        fs.copyFile(
          path.join(paths.dataRoot, 'staging', chunkFolder, `${tbluHash}.TBLU.meta`),
          path.join(
            paths.dataRoot,
            assignedTemporaryDirectory,
            tbluRPKG,
    ...
    await callRPKGFunction(
      `-hash_meta_to_json "${path.join(paths.dataRoot, assignedTemporaryDirectory, tempRPKG, 'TEMP', `${tempHash}.TEMP.meta`)}"`
    )
    await callRPKGFunction(
      `-hash_meta_to_json "${path.join(paths.dataRoot, assignedTemporaryDirectory, tbluRPKG, 'TBLU', `${tbluHash}.TBLU.meta`)}"`
    ) // Generate the RT files from the binary files
    ...
    for (const patch of patches) {
      ...
      if (!getQuickEntityFromPatchVersion(patch.patchVersion.value)) {
        ...
        await logger.error(
          `Could not find matching QuickEntity version for patch version ${Number(patch.patchVersion.value)}!`
        )
      }

      fs.writeFileSync(
        path.join(paths.dataRoot, assignedTemporaryDirectory, 'patch.json'),
        LosslessJSON.stringify(patch)
      )
    ...
    await callRPKGFunction(
      `-json_to_hash_meta "${path.join(paths.dataRoot, assignedTemporaryDirectory, `${tempHash}.TEMP.meta.JSON`)}"`
    )
    await callRPKGFunction(
      `-json_to_hash_meta "${path.join(paths.dataRoot, assignedTemporaryDirectory, `${tbluHash}.TBLU.meta.JSON`)}"`
    ...
    await logger.debug(
      `Restored patch chain ending in ${patches[patches.length - 1].path} from cache`
    )
  }
  ...
  await Promise.all([
    fs.copyFile(
      path.join(paths.dataRoot, assignedTemporaryDirectory, `${tempHash}.TEMP`),
      path.join(paths.dataRoot, 'staging', chunkFolder, `${tempHash}.TEMP`)
    ),
    fs.copyFile(
      path.join(paths.dataRoot, assignedTemporaryDirectory, `${tempHash}.TEMP.meta`),
      path.join(paths.dataRoot, 'staging', chunkFolder, `${tempHash}.TEMP.meta`)
    ),
    fs.copyFile(
      path.join(paths.dataRoot, assignedTemporaryDirectory, `${tbluHash}.TBLU`),
      path.join(paths.dataRoot, 'staging', chunkFolder, `${tbluHash}.TBLU`)
    ),
    fs.copyFile(
      path.join(paths.dataRoot, assignedTemporaryDirectory, `${tbluHash}.TBLU.meta`),
      path.join(paths.dataRoot, 'staging', chunkFolder, `${tbluHash}.TBLU.meta`)
