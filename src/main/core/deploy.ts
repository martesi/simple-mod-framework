...
const deepMerge = (x: any, y: any) =>
  mergeWith(x, y, (orig, src) => {
...
function resolvePatchWorkerPath(): string {
  ...
  throw new Error(
    `Could not find patchWorker.js or patchWorker.cjs next to ${__dirname} - was it bundled alongside this module?`
  )
}
...
export default async function deploy(
  ...
  for (const mod of config.loadOrder) {
    ...
    if (!resolved) {
      await logger.error(
        `Could not resolve mod ${mod} to its folder in Mods! Try "Rebuild cache" if this mod was added/changed outside the Mod Manager.`
      )
      return
    }
    ...
    if (!resolved.isFrameworkMod) {
      ...
      for (const chunkFolder of fs.readdirSync(path.join(config.modsPath, modFolder))) {
        ...
        for (const contentFile of fs.readdirSync(
          path.join(config.modsPath, modFolder, chunkFolder)
        )) {
          if (
            !(await copyFromCache(
              modFolder,
              path.join(chunkFolder, contentFile),
              path.join(paths.dataRoot, 'temp')
            ))
          ) {
            await callRPKGFunction(
              `-extract_from_rpkg "${path.join(config.modsPath, modFolder, chunkFolder, contentFile)}" -output_path "${path.join(paths.dataRoot, 'temp')}"`
            )
            await copyToCache(
              modFolder,
              path.join(paths.dataRoot, 'temp'),
        ...
        allFiles.forEach((a) =>
          fs.copyFileSync(a, path.join(paths.dataRoot, 'staging', chunkFolder, path.basename(a)))
        )
  ...
  for (const instruction of deployInstructions) {
    ...
    instruction.content.sort((a, b) =>
      (a.order || (a.source === 'disk' ? a.chunk + a.path : a.chunk + a.identifier)).localeCompare(
        b.order || (b.source === 'disk' ? b.chunk + b.path : b.chunk + b.identifier),
        'en-AU',
        {
          numeric: true,
        }
    ...
    if (instruction.content.some((a) => a.type === 'contract.json')) {
      ...
      if (
        !(await copyFromCache(
          ...
          'contractsORES',
          path.join(paths.dataRoot, 'temp2')
        ))
      ) {
        ...
        if (
          !fs.existsSync(path.join(paths.dataRoot, 'staging', 'chunk0', '002B07020D21D727.ORES'))
        ) {
          await callRPKGFunction(
          ...
          fs.copyFileSync(
            path.join(paths.dataRoot, 'staging', 'chunk0', '002B07020D21D727.ORES'),
            path.join(paths.dataRoot, 'temp2', contractsORESChunk, 'ORES', '002B07020D21D727.ORES')
          ) // Use the staging one (for mod compat - one mod can extract, patch and build, then the next can patch that one instead)
          fs.copyFileSync(
            path.join(paths.dataRoot, 'staging', 'chunk0', '002B07020D21D727.ORES.meta'),
            path.join(
              paths.dataRoot,
              'temp2',
              contractsORESChunk,
        ...
        execCommand(
          `"${thirdParty('OREStool.exe')}" "${path.join(paths.dataRoot, 'temp2', contractsORESChunk, 'ORES', '002B07020D21D727.ORES')}"`
        )

        await callRPKGFunction(
          `-hash_meta_to_json "${path.join(paths.dataRoot, 'temp2', contractsORESChunk, 'ORES', '002B07020D21D727.ORES.meta')}"`
        )
      }
    ...
    if (instruction.content.some((a) => a.type === 'unlockables.json')) {
      ...
      if (
        !(await copyFromCache(
          instruction.cacheFolder,
          'unlockablesORES',
          path.join(paths.dataRoot, 'temp3')
        ))
      ) {
        ...
        if (
          !fs.existsSync(path.join(paths.dataRoot, 'staging', 'chunk0', '0057C2C3941115CA.ORES'))
        ) {
          await callRPKGFunction(
          ...
          fs.copyFileSync(
            path.join(paths.dataRoot, 'staging', 'chunk0', '0057C2C3941115CA.ORES'),
            path.join(
              paths.dataRoot,
              'temp3',
              unlockablesORESChunk,
              'ORES',
        ...
        execCommand(
          `"${thirdParty('OREStool.exe')}" "${path.join(paths.dataRoot, 'temp3', unlockablesORESChunk, 'ORES', '0057C2C3941115CA.ORES')}"`
        )
      }

      const rawUnlockablesContent: { Id: string }[] = JSON.parse(
        fs.readFileSync(
          path.join(
            paths.dataRoot,
            'temp3',
    ...
    if (instruction.content.some((a) => a.type === 'repository.json')) {
      ...
      if (
        !(await copyFromCache(
          instruction.cacheFolder,
          'repositoryREPO',
          path.join(paths.dataRoot, 'temp4')
        ))
      ) {
        ...
        fs.copySync(
          path.join(paths.dataRoot, 'temp', repositoryRPKG, 'REPO'),
          path.join(paths.dataRoot, 'temp4', repositoryRPKG, 'REPO')
        )
        fs.emptyDirSync(path.join(paths.dataRoot, 'temp'))
      }

      const rawRepositoryContent: { [x: string]: unknown }[] = JSON.parse(
        fs.readFileSync(
          path.join(paths.dataRoot, 'temp4', repositoryRPKG, 'REPO', '00204D1AFD76AB13.REPO'),
          'utf8'
        )
      )
    ...
    for (const content of instruction.content) {
      ...
      if (content.source === 'disk')
        void logger.verbose(`Staging ${content.type} file ${content.path}`)
      if (content.source === 'virtual')
        void logger.verbose(`Staging virtual ${content.type} file ${content.identifier}`)
      ...
      switch (content.type) {
        case 'entity.json': {
          ...
          entityContent = LosslessJSON.parse(
            String(
              content.source === 'disk'
                ? fs.readFileSync(content.path)
                : await content.content.text()
            )
          )
          ...
          try {
            if (!getQuickEntityFromVersion(entityContent.quickEntityVersion.value)) {
              await logger.error(
                `Could not find matching QuickEntity version for ${Number(entityContent.quickEntityVersion.value)}!`
              )
            }
          } catch {
          ...
          if (+entityContent.quickEntityVersion.value < 3) {
            if (content.source === 'disk') {
              ...
              const comments = Object.entries(entityContent.entities).filter(
                (a) => (a[1] as { type: string | undefined }).type === 'comment'
              )
              ...
              if (!config.developerMode) {
                ...
                await logger.warn(
                  `Automatically upgraded an entity.json file from ${instruction.id} to the latest QuickEntity version.`
                )
              }
            } else {
              await logger.warn(
                `Mod ${instruction.id} emits a virtual QuickEntity JSON with a version less than 3.1 using scripting. This should not be the case.`
              )
            }
          ...
          if (
            !(await copyFromCache(
              ...
              path.join(
                `chunk${content.chunk}`,
                `${path.basename(contentIdentifier).slice(0, 15)}-${await xxhash3(contentIdentifier)}`
              ),
            ...
            if (content.source === 'disk') {
              ...
              fs.writeFileSync(
                path.join(paths.dataRoot, 'virtual', 'entity.json'),
                Buffer.from(await content.content.arrayBuffer())
              )
            ...
            await callRPKGFunction(
              `-json_to_hash_meta "${path.join(paths.dataRoot, 'temp', `${entityContent.tempHash}.TEMP.meta.json`)}"`
            )
            await callRPKGFunction(
              `-json_to_hash_meta "${path.join(paths.dataRoot, 'temp', `${entityContent.tbluHash}.TBLU.meta.json`)}"`
            )
            // Generate the binary files from the RT json

            fs.copyFileSync(
              path.join(paths.dataRoot, 'temp', `${entityContent.tempHash}.TEMP`),
              path.join(
                paths.dataRoot,
                'staging',
                `chunk${content.chunk}`,
            ...
            fs.copyFileSync(
              path.join(paths.dataRoot, 'temp', `${entityContent.tbluHash}.TBLU`),
              path.join(
                paths.dataRoot,
                'staging',
                `chunk${content.chunk}`,
        ...
        case 'entity.patch.json': {
          ...
          entityContent =
            content.source === 'disk'
              ? LosslessJSON.parse(fs.readFileSync(content.path, 'utf8'))
              : LosslessJSON.parse(await content.content.text())
          ...
          if (+entityContent.patchVersion.value < 6) {
            if (content.source === 'disk') {
              ...
              const tempRPKG = await rpkgInstance.getRPKGOfHash(
                config.runtimePath,
                entityContent.tempHash
              )
              const tbluRPKG = await rpkgInstance.getRPKGOfHash(
                config.runtimePath,
              ...
              await callRPKGFunction(
                `-hash_meta_to_json "${path.join(paths.dataRoot, 'qn-update', tempRPKG, 'TEMP', `${entityContent.tempHash}.TEMP.meta`)}"`
              )
              await callRPKGFunction(
                `-hash_meta_to_json "${path.join(paths.dataRoot, 'qn-update', tbluRPKG, 'TBLU', `${entityContent.tbluHash}.TBLU.meta`)}"`
              )
              ...
              if (+entityContent.patchVersion.value < 3) {
                await getQuickEntityFromPatchVersion(entityContent.patchVersion.value).convert(
                  ...
                  path.join(
                    ...
                    tempRPKG,
                    'TEMP',
                    `${entityContent.tempHash}.TEMP.json`
                  ),
                  path.join(
                    paths.dataRoot,
                    'qn-update',
                    tempRPKG,
                    'TEMP',
                    `${entityContent.tempHash}.TEMP.meta.JSON`
                  ),
                  path.join(
                    paths.dataRoot,
                    'qn-update',
                    tbluRPKG,
                    'TBLU',
                    `${entityContent.tbluHash}.TBLU.json`
                  ),
                  path.join(
                    paths.dataRoot,
                    'qn-update',
                    tbluRPKG,
                    'TBLU',
                    `${entityContent.tbluHash}.TBLU.meta.JSON`
                  ),
              ...
              if (!config.developerMode) {
                ...
                await logger.warn(
                  `Automatically upgraded an entity.patch.json file from ${instruction.id} to the latest QuickEntity version.`
                )
              }
            } else {
              await logger.warn(
                `Mod ${instruction.id} emits a virtual QuickEntity patch JSON with a patch version less than 6 using scripting. This should not be the case.`
              )
            }
          }
        ...
        case 'unlockables.json': {
          ...
          entityContent =
            content.source === 'disk'
              ? JSON.parse(fs.readFileSync(content.path, 'utf8'))
              : JSON.parse(await content.content.text())
        ...
        case 'contract.json': {
          ...
          if (!Object.values(contractsORESContent).includes(entityContent.Metadata.Id)) {
            ...
            contractHash = Object.entries(contractsORESContent).find(
              (a) => a[1] === entityContent.Metadata.Id
            )?.[0]
          }

          fs.writeFileSync(
            path.join(paths.dataRoot, 'staging', 'chunk0', `${contractHash}.JSON`),
            LosslessJSON.stringify(entityContent)
        ...
        case 'JSON.patch.json': {
          ...
          if (
            entityContent.file === '004F4B738474CEAD' &&
            (entityContent.patch as any[]).every(
              (a) => a.op === 'add' && a.path === '/Root/Children/-'
            )
          ) {
            if (content.source === 'disk') {
              ...
              if (contractsToAdd.every((a) => allContracts[a.Id])) {
                ...
                if (!config.developerMode) {
                  ...
                  await logger.warn(
                    `Updated a destination enabling JSON from ${instruction.id} to use the contract SMF key.`
                  )
                }
              ...
              await logger.warn(
                `Mod ${instruction.id} emits a destination-enabling JSON.patch.json using scripting. This should not be the case.`
              )
            }
          }
          ...
          if (
            ...
            if (entityContent.type === 'ORES') {
              execCommand(
                `"${thirdParty('OREStool.exe')}" "${path.join(paths.dataRoot, 'temp', rpkgOfFile, fileType, `${entityContent.file}.${fileType}`)}"`
              )
              fs.rmSync(
                path.join(
                  paths.dataRoot,
            ...
            if (entityContent.type === 'ORES' && Array.isArray(fileContent)) {
              ...
              fileContent = Object.fromEntries(
                fileContent.map((a: { [x: string]: unknown }) => [a.ID_, a])
            ...
            if (
              (entityContent.type === 'ORES' &&
                Object.prototype.toString.call(fileContent) === '[object Object]') ||
            ...
            if (entityContent.type === 'ORES') {
              ...
              execCommand(
                `"${thirdParty('OREStool.exe')}" "${path.join(paths.dataRoot, 'temp', rpkgOfFile, fileType, `${entityContent.file}.${fileType}.json`)}"`
              )
            } else {
              fs.writeFileSync(
                path.join(
        ...
        case 'material.json': {
          ...
          const materialCacheKey = path.join(
            `chunk${content.chunk}`,
            `material-${path.basename(contentIdentifier).slice(0, 15)}-${materialHash}`
          )
          const materialTempDir = path.join(
            paths.dataRoot,
            'temp',
            'material',
            `chunk${content.chunk}`,
          ...
          if (
            ...
            if (content.source === 'disk') {
              ...
              fs.ensureDirSync(path.join(paths.dataRoot, 'virtual'))
              fs.writeFileSync(
                path.join(paths.dataRoot, 'virtual', 'material.json'),
                Buffer.from(await content.content.arrayBuffer())
              )
            ...
            await callRPKGFunction(
              `-json_to_material "${contentFilePath}" -output_path "${materialTempDir}"`
            )
        ...
        case 'texture.tga': {
          ...
          if (
            ...
            if (
              (content.source === 'disk' &&
                path.basename(content.path).split('.')[0].split('~').length > 1) ||
              (content.source === 'virtual' && content.extraInformation.texdHash)
            ) {
              // TEXT and TEXD
              ...
              if (content.source === 'disk') {
                ...
                fs.ensureDirSync(path.join(paths.dataRoot, 'virtual'))
                fs.writeFileSync(
                  path.join(paths.dataRoot, 'virtual', 'texture.tga'),
                  Buffer.from(await content.content.arrayBuffer())
                )
                fs.writeFileSync(
                  path.join(paths.dataRoot, 'virtual', 'texture.tga.meta'),
                  Buffer.from(await content.extraInformation.textureMeta!.arrayBuffer())
                )
              ...
              fs.writeFileSync(
                ...
                JSON.stringify({
                  hash_value:
                    content.source === 'disk'
                      ? path.basename(content.path).split('.')[0].split('~')[0]
                      : content.extraInformation.textHash,
                  ...
                  hash_reference_data: [
                    {
                      hash:
                        content.source === 'disk'
                          ? path.basename(content.path).split('.')[0].split('~')[1]
                          : content.extraInformation.texdHash,
                      flag: '9F',
                    },
                  ],
                })
        ...
        case 'sfx.wem': {
          if (
            !WWEVpatches[
              content.source === 'disk'
                ? path.basename(content.path).split('.')[0].split('~')[0]
                : content.extraInformation.wwevHash!
            ]
          ) {
            WWEVpatches[
              content.source === 'disk'
          ...
          WWEVpatches[
            content.source === 'disk'
              ? path.basename(content.path).split('.')[0].split('~')[0]
              : String(content.extraInformation.wwevHash)
          ].push({
            index:
              content.source === 'disk'
                ? path.basename(content.path).split('.')[0].split('~')[1]
                : String(content.extraInformation.wwevElement),
        ...
        case 'delta': {
          ...
          const runtimeID =
            content.source === 'disk'
              ? path.basename(content.path).split('.')[0].split('~')[0]
              : content.extraInformation.runtimeID!
          const fileType =
            content.source === 'disk'
              ? path.basename(content.path).split('.')[0].split('~')[1]
              : content.extraInformation.fileType!
          ...
          if (
            ...
            if (content.source === 'disk') {
              ...
              fs.ensureDirSync(path.join(paths.dataRoot, 'virtual'))
              fs.writeFileSync(
                path.join(paths.dataRoot, 'virtual', 'patch.delta'),
                Buffer.from(await content.content.arrayBuffer())
              )
        ...
        case 'rtlv.json': {
          ...
          if (
            ...
            if (content.source === 'disk') {
              ...
              fs.ensureDirSync(path.join(paths.dataRoot, 'virtual'))
              fs.writeFileSync(
                path.join(paths.dataRoot, 'virtual', content.type),
                Buffer.from(await content.content.arrayBuffer())
              )
          ...
          fs.copyFileSync(
            path.join(paths.dataRoot, 'temp', `chunk${content.chunk}`, `${hash}.${binaryType}`),
            path.join(paths.dataRoot, 'staging', `chunk${content.chunk}`, `${hash}.${binaryType}`)
          )
          fs.copyFileSync(
            path.join(
              paths.dataRoot,
        ...
        default: // Copy the file to the staging directory; we don't cache these for obvious reasons
          if (
            (content.source === 'disk'
              ? path.basename(content.path).split('.').slice(1).join('.')
              : content.extraInformation.fileType!
            ).length === 4 ||
            (content.source === 'disk'
              ? path.basename(content.path).split('.').slice(1).join('.')
            ...
            fs.writeFileSync(
              content.source === 'disk'
                ? path.join(
                    ...
                    'staging',
                    `chunk${content.chunk}`,
                    path.basename(content.path)
                  )
                : path.join(
                    paths.dataRoot,
                    'staging',
                    `chunk${content.chunk}`,
                    `${content.extraInformation.runtimeID!}.${content.extraInformation.fileType!}`
                  ),
              content.source === 'disk'
                ? fs.readFileSync(content.path)
                : Buffer.from(await content.content.arrayBuffer())
    ...
    if (instruction.content.some((a) => a.type === 'contract.json')) {
      ...
      if (contractsCacheInvalid) {
        ...
        await callRPKGFunction(
          `-json_to_hash_meta "${path.join(paths.dataRoot, 'temp2', contractsORESChunk, 'ORES', '002B07020D21D727.ORES.meta.JSON')}"`
        ...
        execCommand(
          `"${thirdParty('OREStool.exe')}" "${path.join(paths.dataRoot, 'temp2', contractsORESChunk, 'ORES', '002B07020D21D727.ORES.json')}"`
        ) // Rebuild the ORES
    ...
    if (unlockablesORESChunk) {
      ...
      if (unlockablesCacheInvalid) {
        const unlockablesToWrite = Object.entries(unlockablesORESContent).map(([id, entry]) => ({
          ...entry,
          Id: (entry as { Id?: string }).Id || id,
        }))
        ...
        fs.writeFileSync(
          ...
          JSON.stringify(unlockablesToWrite)
        )
        fs.rmSync(
          path.join(paths.dataRoot, 'temp3', unlockablesORESChunk, 'ORES', '0057C2C3941115CA.ORES')
        )
        execCommand(
          `"${thirdParty('OREStool.exe')}" "${path.join(paths.dataRoot, 'temp3', unlockablesORESChunk, 'ORES', '0057C2C3941115CA.ORES.json')}"`
        ) // Rebuild the ORES
      ...
      lastServerSideStates.unlockables = JSON.parse(
        fs.readFileSync(
          path.join(
            paths.dataRoot,
            'temp3',
    ...
    if (repositoryRPKG) {
      ...
      if (repositoryCacheInvalid) {
        const repositoryToWrite = Object.entries(repositoryContent).map(([id, entry]) => ({
          ...entry,
          ID_: (entry as { ID_?: string }).ID_ || id,
        }))

        await callRPKGFunction(
          `-hash_meta_to_json "${path.join(paths.dataRoot, 'temp4', repositoryRPKG, 'REPO', '00204D1AFD76AB13.REPO.meta')}"`
        )
        ...
        for (const repoItem of repositoryToWrite as {
          ID_: string
          Runtime?: string
          Image?: string
        }[]) {
          ...
          if (repoItem.Runtime) {
            if (
              !metaContent.hash_reference_data.find(
                (a: { hash: string }) =>
                  a.hash === parseInt(repoItem.Runtime!, 10).toString(16).toUpperCase()
              )
            ) {
              metaContent.hash_reference_data.push({
                hash: parseInt(repoItem.Runtime, 10).toString(16).toUpperCase(),
                flag: '9F',
          ...
          if (repoItem.Image) {
            ...
            if (
              !metaContent.hash_reference_data.find((a: { hash: string }) => a.hash === imageHash)
            ) {
        ...
        fs.rmSync(
          path.join(paths.dataRoot, 'temp4', repositoryRPKG, 'REPO', '00204D1AFD76AB13.REPO.meta')
        )
        await callRPKGFunction(
          `-json_to_hash_meta "${path.join(paths.dataRoot, 'temp4', repositoryRPKG, 'REPO', '00204D1AFD76AB13.REPO.meta.JSON')}"`
        ) // Add all runtimes to REPO depends

        fs.writeFileSync(
          path.join(paths.dataRoot, 'temp4', repositoryRPKG, 'REPO', '00204D1AFD76AB13.REPO'),
          JSON.stringify(repositoryToWrite)
        )
      ...
      fs.copyFileSync(
        path.join(paths.dataRoot, 'temp4', repositoryRPKG, 'REPO', '00204D1AFD76AB13.REPO'),
        path.join(paths.dataRoot, 'staging', 'chunk0', '00204D1AFD76AB13.REPO')
      )
      fs.copyFileSync(
        path.join(paths.dataRoot, 'temp4', repositoryRPKG, 'REPO', '00204D1AFD76AB13.REPO.meta'),
        path.join(paths.dataRoot, 'staging', 'chunk0', '00204D1AFD76AB13.REPO.meta')
      )
    ...
    for (const [rpkg, data] of Object.entries(instruction.rpkgTypes)) {
      if (data.type === 'base') {
        ...
        if (typeof data.chunkMeta === 'string') {
          fs.copyFileSync(
            data.chunkMeta,
            path.join(paths.dataRoot, 'staging', rpkg, `${rpkg}.meta`)
          )
        } else if (data.chunkMeta instanceof Blob) {
          fs.writeFileSync(
            path.join(paths.dataRoot, 'staging', rpkg, `${rpkg}.meta`),
            Buffer.from(await data.chunkMeta.arrayBuffer())
          )
        }
      }
    ...
    if (instruction.blobs.length) {
      ...
      execCommand(
        `"${thirdParty('OREStool.exe')}" "${path.join(paths.dataRoot, 'temp', oresChunk, 'ORES', '00858D45F5F9E3CA.ORES')}"`
      )
      const oresContent = JSON.parse(
        fs.readFileSync(
          path.join(paths.dataRoot, 'temp', oresChunk, 'ORES', '00858D45F5F9E3CA.ORES.JSON'),
          'utf8'
        )
      )

      await callRPKGFunction(
        `-hash_meta_to_json "${path.join(paths.dataRoot, 'temp', oresChunk, 'ORES', '00858D45F5F9E3CA.ORES.meta')}"`
      )
      const metaContent = JSON.parse(
        fs.readFileSync(
          path.join(paths.dataRoot, 'temp', oresChunk, 'ORES', '00858D45F5F9E3CA.ORES.meta.JSON'),
          'utf8'
        )
      )
      ...
      for (const blob of instruction.blobs) {
        ...
        if (!blob.blobHash) {
          if (
            (blob.source === 'disk'
              ? path.extname(blob.filePath).slice(1)
              : blob.filetype
            ).startsWith('jp') ||
            ...
            path.join(
              ...
              `${blobHash}.${
                path.extname(blob.filePath).slice(1) === 'json'
                  ? 'JSON'
                  : path.extname(blob.filePath).slice(1).startsWith('jp') ||
                      path.extname(blob.filePath).slice(1) === 'png'
                    ? 'GFXI'
      ...
      await callRPKGFunction(
        `-json_to_hash_meta "${path.join(paths.dataRoot, 'temp', oresChunk, 'ORES', '00858D45F5F9E3CA.ORES.meta.JSON')}"`
      )
      ...
      execCommand(
        `"${thirdParty('OREStool.exe')}" "${path.join(paths.dataRoot, 'temp', oresChunk, 'ORES', '00858D45F5F9E3CA.ORES.json')}"`
      )

      // Copy the ORES to the staging directory
      fs.copyFileSync(
        path.join(paths.dataRoot, 'temp', oresChunk, 'ORES', '00858D45F5F9E3CA.ORES'),
        path.join(paths.dataRoot, 'staging', 'chunk0', '00858D45F5F9E3CA.ORES')
      )
      fs.copyFileSync(
        path.join(paths.dataRoot, 'temp', oresChunk, 'ORES', '00858D45F5F9E3CA.ORES.meta'),
        path.join(paths.dataRoot, 'staging', 'chunk0', '00858D45F5F9E3CA.ORES.meta')
      )
    ...
    if (instruction.manifestSources.dependencies) {
      ...
      for (const dependency of instruction.manifestSources.dependencies) {
        ...
        const dependencyPortFromChunk1 =
          typeof dependency === 'string' ? false : !!dependency.portFromChunk1

        if (
          !doneHashes.some((a) => a.id === dependencyID && a.chunk === dependencyChunk) ||
          (doneHashes
            .filter((a) => a.id === dependencyID && a.chunk === dependencyChunk)
            .every((a) => !a.portChunk1) &&
            dependencyPortFromChunk1)
        ) {
          doneHashes.push({
            id: dependencyID,
          ...
          if (
            fs.existsSync(
              path.join(
                ...
                'cache',
                'global',
                path.join('dependencies', `${dependencyID}-${dependencyPortFromChunk1 ? 1 : 0}`)
              )
            )
          ) {
    ...
    if (instruction.manifestSources.localisation) {
      for (const language of Object.keys(
        instruction.manifestSources.localisation
      ) as (keyof ManifestOptionData['localisation'])[]) {
    ...
    if (instruction.manifestSources.localisationOverrides) {
      for (const locrHash of Object.keys(instruction.manifestSources.localisationOverrides)) {
        ...
        for (const language of Object.keys(
          instruction.manifestSources.localisationOverrides[locrHash]
        ) as (keyof ManifestOptionData['localisation'])[]) {
          for (const string of Object.entries(
            instruction.manifestSources.localisationOverrides[locrHash][language]
          )) {
    ...
    if (instruction.manifestSources.localisedLines) {
      ...
      for (const lineHash of Object.keys(instruction.manifestSources.localisedLines)) {
        ...
        if (
          !(await copyFromCache(
            instruction.cacheFolder,
            path.join('localisedLines', lineHash),
            path.join(paths.dataRoot, 'temp')
          ))
        ) {
          fs.writeFileSync(
          ...
          await callRPKGFunction(
            `-json_to_hash_meta "${path.join(paths.dataRoot, 'temp', 'chunk0', `${lineHash}.LINE.meta.JSON`)}"`
          ) // Rebuild the meta
  ...
  if (contractsToAddToDestinations.length) {
    ...
    if (!(await copyFromCache('global', destinationsCacheKey, destinationsOutputDir))) {
      ...
      const registry = fs.readJSONSync(
        path.join(paths.dataRoot, 'temp', rpkgOfDestinations, 'JSON', '004F4B738474CEAD.JSON')
      )
      ...
      for (const { id, before, after, context } of contractsToAddToDestinations) {
        ...
        if (before) {
          ...
          registry.Root.Children.splice(
            registry.Root.Children.findIndex((a: { Id: string }) => a.Id === after) + 1,
            0,
            {
              Id: id,
              _comment: 'Automatically added by SMF.',
    ...
    fs.copyFileSync(
      path.join(destinationsOutputDir, '004F4B738474CEAD.JSON'),
      path.join(paths.dataRoot, 'staging', 'chunk0', '004F4B738474CEAD.JSON')
    )
  ...
  for (const entry of Object.entries(WWEVpatches)) {
    ...
    if (
      ...
      const workingPath = path.join(
        ...
        'WWEV',
        `${rpkgOfWWEV}.rpkg`,
        fs.readdirSync(path.join(paths.dataRoot, 'temp', 'WWEV', `${rpkgOfWWEV}.rpkg`))[0]
      )
      ...
      for (const patch of entry[1]) {
        if (typeof patch.content === 'string') {
          ...
          fs.writeFileSync(
            path.join(workingPath, 'wem', `${patch.index}.wem`),
            Buffer.from(await patch.content.arrayBuffer())
          ) // Copy the wem
        }
      }
    ...
    fs.copyFileSync(
      ...
      path.join(paths.dataRoot, 'staging', entry[1][0].chunk, `${WWEVhash}.WWEV`)
    )
    fs.copyFileSync(
      path.join(workingPath, `${WWEVhash}.WWEV.meta`),
      path.join(paths.dataRoot, 'staging', entry[1][0].chunk, `${WWEVhash}.WWEV.meta`)
  ...
  if (localisation.length) {
    ...
    if (
      ...
      await callRPKGFunction(
        `-hash_meta_to_json "${path.join(paths.dataRoot, 'temp', `${localisationFileRPKG}`, 'LOCR', '00F5817876E691F1.LOCR.meta')}"`
      )
      ...
      const locrFileContent: HMLanguageToolsLOCR = JSON.parse(
        fs.readFileSync(
          path.join(
            paths.dataRoot,
            'temp',
    ...
    fs.copyFileSync(
      path.join(
        ...
        'LOCR',
        '00F5817876E691F1.LOCR.meta'
      ),
      path.join(paths.dataRoot, 'staging', 'chunk0', '00F5817876E691F1.LOCR.meta')
    )
  ...
  if (Object.keys(localisationOverrides).length) {
    ...
    for (const locrHash of Object.keys(localisationOverrides)) {
      ...
      fs.ensureDirSync(
        path.join(paths.dataRoot, 'staging', localisationFileRPKG.replace(/patch[0-9]*/gi, ''))
      )

      if (
        !(await copyFromCache(
          'global',
          path.join('LOCR', locrHash),
          path.join(paths.dataRoot, 'temp')
        ))
      ) {
        // we need to re-deploy the localisation files OR the localisation files couldn't be copied from cache
        ...
        await callRPKGFunction(
          `-hash_meta_to_json "${path.join(paths.dataRoot, 'temp', localisationFileRPKG, 'LOCR', `${locrHash}.LOCR.meta`)}"`
        )
        ...
        for (const item of localisationOverrides[locrHash]) {
          ...
          toMerge[parseInt(item.locString, 10).toString(16)] = item.text
        ...
        fs.writeFileSync(
          path.join(paths.dataRoot, 'temp', localisationFileRPKG, 'LOCR', `${locrHash}.LOCR.JSON`),
          JSON.stringify(locrToWrite)
        )
      ...
      fs.copyFileSync(
        path.join(paths.dataRoot, 'temp', localisationFileRPKG, 'LOCR', `${locrHash}.LOCR.meta`),
        path.join(
          paths.dataRoot,
          'staging',
          localisationFileRPKG.replace(/patch[0-9]*/gi, ''),
  ...
  if (config.skipIntro || thumbs.length) {
    ...
    if (!(await copyFromCache('global', thumbsCacheKey, thumbsOutputDir))) {
      ...
      if (!fs.existsSync(path.join(paths.dataRoot, 'cleanThumbs.dat'))) {
        ...
        fs.copyFileSync(
          ...
          path.join(paths.dataRoot, 'cleanThumbs.dat')
        )
      }

      execCommand(
        `"${thirdParty('h6xtea.exe')}" -d --src "${path.join(paths.dataRoot, 'cleanThumbs.dat')}" --dst "${path.join(paths.dataRoot, 'temp', 'thumbs.dat.decrypted')}"`
      ) // Decrypt thumbs

      let thumbsContent = fs
        .readFileSync(path.join(paths.dataRoot, 'temp', 'thumbs.dat.decrypted'), 'utf8')
        .split(/\r?\n/)
        .join('\n')
      ...
      execCommand(
        `"${thirdParty('h6xtea.exe')}" -e --src "${path.join(paths.dataRoot, 'temp', 'thumbs.dat.decrypted')}" --dst "${path.join(paths.dataRoot, 'temp', 'thumbs.dat.decrypted.encrypted')}"`
      ) // Encrypt thumbs
    ...
    atomicCopyFileSync(
      path.join(thumbsOutputDir, 'thumbs.dat'),
      config.outputToSeparateDirectory
        ? path.join(paths.dataRoot, 'Output', 'thumbs.dat')
        : path.join(config.retailPath, 'thumbs.dat')
    ) // Output thumbs
  ...
  if (!fs.existsSync(path.join(paths.dataRoot, 'cleanPackageDefinition.txt'))) {
    ...
    fs.copyFileSync(
      ...
      path.join(paths.dataRoot, 'cleanPackageDefinition.txt')
    )
  }

  execCommand(
    `"${thirdParty('h6xtea.exe')}" -d --src "${path.join(config.runtimePath, 'packagedefinition.txt')}" --dst "${path.join(paths.dataRoot, 'temp', 'packagedefinitionVersionCheck.txt')}"`
  )
  if (
    !fs
      .readFileSync(path.join(paths.dataRoot, 'temp', 'packagedefinitionVersionCheck.txt'))
      .includes('patchlevel=310')
  ) {
  ...
  // Decrypt PD
  execCommand(
    `"${thirdParty('h6xtea.exe')}" -d --src "${path.join(paths.dataRoot, 'cleanPackageDefinition.txt')}" --dst "${path.join(paths.dataRoot, 'temp', 'packagedefinition.txt.decrypted')}"`
  )

  await logger.verbose('Reading packagedefinition')
  const basePackagedefinitionContent = fs.readFileSync(
    path.join(paths.dataRoot, 'temp', 'packagedefinition.txt.decrypted'),
    'utf8'
  )
  ...
  const packagedefinitionHash = await xxhash3(
    basePackagedefinitionContent + JSON.stringify(packagedefinition)
  )
        ...
        case 'entity':
          if (!packagedefinitionContent.includes(brick.path)) {
            const newPD = packagedefinitionContent.replace(
              new RegExp(
                `@partition name=${brick.partition} parent=(.*?) type=(.*?) patchlevel=310\r\n`
              ),
              (_a, parent, type) =>
                `@partition name=${brick.partition} parent=${parent} type=${type} patchlevel=310\r\n${brick.path}\r\n`
            )

            if (packagedefinitionContent === newPD) {
              await logger.error(
                `Couldn't find packagedefinition partition ${brick.partition} in which to add ${brick.path}!`
              )
            }
  ...
  atomicCopyFileSync(
    path.join(packagedefinitionOutputDir, 'packagedefinition.txt'),
    config.outputToSeparateDirectory
      ? path.join(paths.dataRoot, 'Output', 'packagedefinition.txt')
      : path.join(config.runtimePath, 'packagedefinition.txt')
  ) // Output PD
  ...
  for (const stagingChunkFolder of fs.readdirSync(path.join(paths.dataRoot, 'staging'))) {
    await callRPKGFunction(
      `-generate_rpkg_quickly_from "${path.join(paths.dataRoot, 'staging', stagingChunkFolder)}" -output_path "${path.join(paths.dataRoot, 'staging')}"`
    )

    const outputChunkName =
      allRPKGTypes[stagingChunkFolder] === 'base'
    ...
    try {
      atomicCopyFileSync(
        ...
        config.outputToSeparateDirectory
          ? path.join(paths.dataRoot, 'Output', outputChunkName)
          : path.join(config.runtimePath, outputChunkName)
      )
  ...
  if (!config.outputToSeparateDirectory) {
    ...
    for (const runtimeFile of fs.readdirSync(config.runtimePath)) {
      ...
      try {
        ...
        if (patchMatch) {
          const patchNumber = parseInt(patchMatch[1], 10)
          ...
          if (parseInt(runtimeFile.split('.')[0].slice(5), 10) > 30) {
            fs.removeSync(path.join(config.runtimePath, runtimeFile))
          }
        } else if (runtimeFile !== 'packagedefinition.txt') {
          await logger.warn(
            `${runtimeFile} in your Runtime folder is not from the vanilla game. This might cause issues with SMF - move it elsewhere!`
          )
        }
      } catch {
