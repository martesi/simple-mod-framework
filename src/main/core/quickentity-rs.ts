...
export async function createPatchJSON(original: string, modified: string, output: string) {
  execCommand(
    `"${qnExe()}" patch generate --input1 "${original}" --input2 "${modified}" --output "${output}" --format-fix`
  )
}

export async function applyPatchJSON(original: string, patch: string, output: string) {
  execCommand(
    `"${qnExe()}" patch apply --input "${original}" --patch "${patch}" --output "${output}" --permissive --format-fix`
  )
}
