...
import { MANAGED_FOLDER, ModIndex } from './modIndex'
...
export function registerIpcHandlers(paths: AppPaths): DeployManager {
  ...
  ): ReturnType<typeof getStoredGameInfo> {
  ...
  async function rebuildCurrentModIndex(
    onProgress?: (scanned: number, total: number) => void
  ): Promise<ReturnType<typeof index.list>> {
  ...
  const deployManager = new DeployManager(paths, (progress) =>
    broadcast('deploy:progress', progress)
  )
  ...
  ipcMain.handle(
    'config:getDefaultPaths',
    (): DefaultPaths => resolveDefaultUiPaths(paths, loadSettings(paths))
  )
  ...
  ipcMain.handle('config:merge', async (_event, patch: Partial<Config>): Promise<Config> => {
    ...
    const gamePathChanged =
      settingsPatch.gamePath !== undefined && settingsPatch.gamePath !== gamePathBefore
    const settings = mergeSettings(
      paths,
      gamePathChanged && settingsPatch.gamePlatform === undefined
        ? { ...settingsPatch, gamePlatform: undefined }
        : settingsPatch
    )
    ...
    if (modsDirBefore !== undefined && getModsDir() !== modsDirBefore) {
      invalidateModsConfigCache()
      await index.rebuildInWorker((scanned, total) =>
        broadcast('mods:cacheProgress', { scanned, total })
      )
    ...
    if (gamePathChanged || tempDirChanged) {
      const list = await rebuildCurrentModIndex((scanned, total) =>
        broadcast('mods:cacheProgress', { scanned, total })
      )
  ...
  ipcMain.handle(
    ...
    async (
      ...
      const detection = deriveGamePathInfoUncached(result.filePaths[0], paths, {
        prepareMicrosoftThumbs: persist,
      })
      if (!detection.ok) {
      ...
      const effectiveDetection =
        explicitPlatform !== undefined ? { ...detection, platform: explicitPlatform } : detection
          ...
          config: toUiConfig(
            {
              ...
              gamePlatform: effectiveDetection.platform,
            },
            getModsConfig(),
            paths,
            effectiveDetection
  ...
  ipcMain.handle(
    ...
    async (event, options?: { title?: string }): Promise<string | null> => {
      const win = BrowserWindow.fromWebContents(event.sender)
      const result = await dialog.showOpenDialog(win ?? undefined!, {
  ...
  ipcMain.handle(
    'mods:previewFolder',
    (_event, dir: string): { exists: boolean; count: number } => {
      if (!dir || !existsSync(dir)) return { exists: false, count: 0 }
      try {
        const count = readdirSync(dir).filter(
          (f) => f !== MANAGED_FOLDER && statSync(join(dir, f)).isDirectory()
        ).length
  ...
  ipcMain.handle('mods:list', async (event) => {
    ...
    if (!index.isBuilt) {
      await index.loadOrRebuild((scanned, total) =>
        event.sender.send('mods:cacheProgress', { scanned, total })
      )
    }
  ...
  ipcMain.handle(
    ...
    (
      event,
      { taskId, file }: { taskId: string; file: { name: string; size: number; path: string } }
    ) => {
      ensureDb()
      ...
      void runAddModTask(
        paths,
        resolveTempDir(paths, loadSettings(paths)),
        getModsDir(),
        index,
        taskId,
        file.path,
  ...
  ipcMain.handle('mods:remove', (_event, modId: string): { ok: boolean; reason?: string } => {
    ...
    if (deployManager.isActive()) {
      return {
        ok: false,
        reason:
          "A deploy is currently running. Deploy reads mod folders throughout the run, so mods can't be removed until it finishes.",
      }
    }
  ...
  ipcMain.handle('deploy:cancel', (_event, snapshotId: string): { ok: boolean; error?: string } =>
    deployManager.cancel(snapshotId)
  )
  ...
  ipcMain.handle(
    'deploy:analyseMod',
    async (_event, modId: string): Promise<{ ok: boolean; error?: string }> => {
      ensureDb()
