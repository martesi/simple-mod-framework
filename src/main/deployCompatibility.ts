...
const MOD_REFERENCE_FIELDS = ["requirements", "incompatibilities", "loadBefore", "loadAfter"] as const
 ...
 */
export function mergeDeployCompatibilityOptionData(target: ManifestOptionData, option: ManifestOptionData): void {
...
export type DeployCompatibilityInstruction = Pick<DeployInstruction, "id" | "name" | "version"> & {
	manifestSources: Pick<DeployInstruction["manifestSources"], "supportedPlatforms" | "requirements" | "incompatibilities" | "loadBefore" | "loadAfter">
}
...
export type DeployCompatibilityResult = { ok: true } | { ok: false; errors: string[]; message: string }

function displayName(instruction: DeployCompatibilityInstruction): string {
	return instruction.name === instruction.id ? instruction.id : `${instruction.name} (${instruction.id})`
}
...
function referenceApplies(
	...
	if (!validRange(range)) {
		errors.push(`${displayName(source)} declares invalid semantic-version range "${range}" for ${relationship} ${id}. Update or reinstall ${source.id} with a corrected manifest.`)
		return false
	}

	if (!target) {
		errors.push(`${displayName(source)} declares ${relationship} ${id} in version range "${range}", but ${id}'s version metadata is unavailable. Install and enable the framework mod, then rebuild its cache.`)
		return false
	}

	if (!valid(target.version)) {
		errors.push(`${displayName(target)} has invalid semantic version "${target.version}", needed to check ${relationship} declared by ${source.id}. Update or reinstall ${target.id}.`)
		return false
	}
 ...
 */
export function validateDeployCompatibility(input: DeployCompatibilityInput): DeployCompatibilityResult {
	const errors: string[] = []
	...
	const instructions = new Map(input.instructions.map((instruction) => [instruction.id, instruction]))

	for (const source of input.instructions) {
		const metadata = source.manifestSources

		if (metadata.supportedPlatforms?.length && !metadata.supportedPlatforms.includes(input.platform as Platform)) {
			errors.push(`${displayName(source)} supports only ${describePlatforms(metadata.supportedPlatforms)}, but this game install is ${input.platform}. Disable ${source.id} or use a supported game platform.`)
		}
		...
		for (const requirement of metadata.requirements ?? []) {
			...
			if (!enabled.has(id)) {
				if (installed.has(id)) {
					errors.push(`${displayName(source)} requires ${id}${range ? ` in version range "${range}"` : ""}, but ${id} is installed and disabled. Enable ${id} and deploy again.`)
				} else {
					errors.push(`${displayName(source)} requires ${id}${range ? ` in version range "${range}"` : ""}, but it is not installed. Install and enable ${id}, then deploy again.`)
				}
				if (range !== undefined && !validRange(range)) {
					errors.push(`${displayName(source)} declares invalid semantic-version range "${range}" for requirement ${id}. Update or reinstall ${source.id} with a corrected manifest.`)
				}
				continue
			}
			...
			if (!referenceApplies(requirement, source, target, "requirement", errors)) {
				if (range !== undefined && validRange(range) && target && valid(target.version)) {
					errors.push(`${displayName(source)} requires ${id} in version range "${range}", but enabled version ${target.version} does not match. Install a compatible version of ${id}.`)
				}
			}
		}
		...
		for (const incompatibility of metadata.incompatibilities ?? []) {
			...
			if (!enabled.has(id)) {
				if (range !== undefined && !validRange(range)) {
					errors.push(`${displayName(source)} declares invalid semantic-version range "${range}" for incompatibility ${id}. Update or reinstall ${source.id} with a corrected manifest.`)
				}
				continue
			}
			...
			if (referenceApplies(incompatibility, source, target, "incompatibility with", errors)) {
				errors.push(`${displayName(source)} is incompatible with ${id}${range ? ` versions matching "${range}" (enabled: ${target?.version ?? "unknown"})` : ""}. Disable either ${source.id} or ${id}.`)
			}
		}
		...
		const checkLoadOrder = (reference: ModReference, direction: "before" | "after"): void => {
			...
			if (!enabled.has(id)) {
				if (range !== undefined && !validRange(range)) {
					errors.push(`${displayName(source)} declares invalid semantic-version range "${range}" for load-${direction} rule targeting ${id}. Update or reinstall ${source.id} with a corrected manifest.`)
				}
				return
			}

			const target = instructions.get(id)
			if (!referenceApplies(reference, source, target, `load-${direction} rule targeting`, errors)) return

			const targetIndex = order.get(id)
			if (targetIndex === undefined) return
			const correctlyOrdered = direction === "before" ? sourceIndex < targetIndex : sourceIndex > targetIndex
			if (!correctlyOrdered) {
				errors.push(`${displayName(source)} must load ${direction} ${id}${range ? ` when ${id} matches "${range}"` : ""}. Move ${source.id} ${direction} ${id} in the mod load order.`)
			}
		}
	...
	return errors.length ? { ok: false, errors, message: formatCompatibilityFailure(errors) } : { ok: true }
}
