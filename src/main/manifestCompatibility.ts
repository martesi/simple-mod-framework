...
function normalizeAliases(source: JsonObject): JsonObject {
  ...
  if (Object.hasOwn(normalized, 'packageDefinition'))
    normalized.packagedefinition = normalized.packageDefinition
  ...
  for (const [id, translationsValue] of Object.entries(source)) {
    ...
    for (const [language, text] of Object.entries(translations)) {
      if (!LANGUAGE_SET.has(language))
        fail(`${path}.${id}.${language}`, `uses unsupported language "${language}"`)
...
export function normalizeModReference(value: unknown, path = '$.reference'): ModReference {
  ...
  if (typeof value === 'string') {
    ...
    if (value.length !== 2)
      fail(path, 'legacy reference tuples must contain exactly an ID and range')
    ;[id, range] = value
...
function normalizePlatforms(value: unknown, path: string): DiskManifest['supportedPlatforms'] {
  ...
  for (let index = 0; index < value.length; index++) {
    ...
    if (/^h[12](?:-|$)/.test(value[index] as string))
      fail(`${path}[${index}]`, `targets unsupported HITMAN version "${value[index]}"`)
  }

  // Presence of a v3 platform makes that representation authoritative over legacy store names.
  const hasV3Platforms = value.some(
    (platform) => typeof platform === 'string' && /^h3(?:-|$)/.test(platform)
  )
  const selected = hasV3Platforms
    ? value.filter((platform) => typeof platform === 'string' && /^h3(?:-|$)/.test(platform))
    : value
  ...
  for (let index = 0; index < selected.length; index++) {
    ...
    else if (platform.startsWith('h3-') && LEGACY_PLATFORM_SET.has(platform.slice(3)))
      mapped = [platform.slice(3) as (typeof LEGACY_PLATFORMS)[number]]
    else if (LEGACY_PLATFORM_SET.has(platform))
      mapped = [platform as (typeof LEGACY_PLATFORMS)[number]]
...
function normalizeScope(value: unknown, path: string): JsonObject {
  ...
  if (normalized.localisation !== undefined)
    normalized.localisation = normalizeLocalisation(normalized.localisation, `${path}.localisation`)
  if (normalized.supportedPlatforms !== undefined)
    normalized.supportedPlatforms = normalizePlatforms(
      normalized.supportedPlatforms,
      `${path}.supportedPlatforms`
    )
  for (const field of REFERENCE_FIELDS) {
    if (normalized[field] !== undefined)
      normalized[field] = normalizeReferences(normalized[field], `${path}.${field}`)
  }
...
function validateRequiredFields(manifest: JsonObject): void {
  ...
  if (!valid(manifest.version as string))
    fail('$.version', `must be a valid semantic version (received "${manifest.version}")`)
  if (!valid(manifest.frameworkVersion as string))
    fail(
      '$.frameworkVersion',
      `must be a valid semantic version (received "${manifest.frameworkVersion}")`
    )
  if (major(manifest.frameworkVersion as string) > major(FRAMEWORK_VERSION)) {
    fail(
      '$.frameworkVersion',
      `targets framework ${manifest.frameworkVersion}, but this build supports framework major ${major(FRAMEWORK_VERSION)} (${FRAMEWORK_VERSION})`
    )
  }
}
...
export function normalizeManifest(raw: unknown): DiskManifest {
  ...
  if (manifest.options !== undefined) {
    ...
    manifest.options = manifest.options.map(
      (option, index) =>
        normalizeScope(option, `$.options[${index}]`) as unknown as DiskManifestOption
    )
  }
...
export function invalidManifestFallback(raw: unknown, folder: string): DiskManifest {
  ...
  const stringOr = (field: string, fallback: string): string =>
    typeof source[field] === 'string' && source[field].trim() ? source[field] : fallback
  const authors = Array.isArray(source.authors)
    ? source.authors.filter((author): author is string => typeof author === 'string')
    : []
