...
function loadCompatibilityInstructions(
  loadOrder: readonly string[]
): { ok: true; instructions: DeployCompatibilityInstruction[] } | { ok: false; error: string } {
  ...
  for (const modId of loadOrder) {
    ...
    if (!mod)
      return {
        ok: false,
        error: `Enabled mod "${modId}" is missing from the index. Rebuild the mod index or remove it from the load order.`,
      }
    if (!mod.isFrameworkMod) continue
    if (mod.valid === false)
      return {
        ok: false,
        error: `Enabled mod "${modId}" has an invalid manifest and cannot be deployed: ${mod.validationError ?? 'fix the manifest and rebuild the index.'}`,
      }

    const build = getModBuild(modId)
    if (build?.status !== 'ready' || !build.deployInstructionJson) {
      return {
        ok: false,
        error: `Compatibility preflight could not read ${modId}'s ready deploy instruction. Rebuild the mod cache and deploy again.`,
      }
    }
    ...
    try {
      ...
      return {
        ok: false,
        error: `Compatibility preflight could not parse ${modId}'s cached deploy instruction. Rebuild the mod cache and deploy again.`,
      }
    }
  }
 ...
 */
export function buildFrameworkConfig(
  paths: AppPaths,
...
async function createEmbeddedCore(
  ...
  toolsRoot: string,
  config: Config,
  onLog: (line: DeployPipelineLogLine) => void
): Promise<Core> {
  const [{ createCore }, { setCurrentCore }] = await Promise.all([
    import('./core/core'),
    import('./core/core-singleton'),
  ])
 ...
 */
export async function runFullDeploy(
  paths: AppPaths,
  settings: AppSettings,
  modsConfig: ModsConfig,
  game: KnownGamePathInfo,
 ...
 */
export async function runAnalyseMod(
  paths: AppPaths,
  settings: AppSettings,
  modsConfig: ModsConfig,
  game: KnownGamePathInfo,
