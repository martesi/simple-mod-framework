import { accessSync, copyFileSync, existsSync, constants as fsConstants } from 'node:fs'
import { join, resolve } from 'node:path'
import { type GamePlatform, isGamePlatform } from '../shared/game'
import { getStoredGameInfo, type StoredGameInfo, setStoredGameInfo } from './db'
import type { AppPaths } from './paths'

export type { GamePlatform } from '../shared/game'

export const UNKNOWN_GAME_PLATFORM_ERROR =
  "The game's storefront could not be inferred. Choose Steam, Epic, or Microsoft in Settings before deploying."
export const INVALID_GAME_PATH_ERROR =
  "No valid game folder is set - open Settings and pick your game's root folder first."
...
export function hasKnownGamePlatform(
  detection: GamePathDetection
): detection is { ok: true } & KnownGamePathInfo {
...
export function deriveGamePathInfo(
  pickedPath: string,
  paths: AppPaths,
 ...
 */
export function deriveGamePathInfoUncached(
  pickedPath: string,
  paths: AppPaths,
  options: { prepareMicrosoftThumbs?: boolean } = {}
): GamePathDetection {
  ...
  if (
    ...
    (existsSync(join(retailPath, 'Retail', 'HITMAN3.exe')) ||
      existsSync(join(retailPath, 'Retail', 'Runtime', 'chunk0.rpkg')))
  ) {
  ...
  if (
    hasNestedRuntime &&
    !existsSync(microsoftConfigPath) &&
    !existsSync(join(retailPath, 'HITMAN3.exe'))
  ) {
    return {
      ok: false,
      error: `MicrosoftGame.Config couldn't be found at "${microsoftConfigPath}".`,
    }
  }
  ...
  if (isMicrosoftLayout && options.prepareMicrosoftThumbs !== false) {
    ...
    if (!existsSync(cleanThumbsDest) && existsSync(cleanThumbsSrc)) {
      try {
        ...
        return {
          ok: false,
          error: `The manager couldn't prepare a clean thumbs.dat copy at "${cleanThumbsDest}".`,
        }
      }
    }
  }
  ...
  return {
    ...
    retailPath,
    runtimePath,
    platform: inferPlatform(pickedPath, retailPath, isMicrosoftLayout),
  }
}
