...
export function parseLocalisationPatch(
  raw: unknown,
  source = 'localisation.patch.json'
): NonNullable<ManifestOptionData['localisationOverrides']> {
  if (!raw || typeof raw !== 'object' || Array.isArray(raw))
    throw new LocalisationPatchError(`${source}: expected an object`)
  ...
  if (typeof resourceId !== 'string' || !/^(?:0x)?[0-9a-f]{16}$/i.test(resourceId)) {
    throw new LocalisationPatchError(
      `${source}: resourceId must be a 16-digit hexadecimal resource ID`
    )
  }
  const lines = input.lines
  if (!lines || typeof lines !== 'object' || Array.isArray(lines))
    throw new LocalisationPatchError(`${source}: lines must be an object`)
  const output: Record<string, Record<string, Record<string, string>>> = {
    [resourceId.replace(/^0x/i, '').toUpperCase()]: {},
  }
  ...
  for (const [lineId, languagesValue] of Object.entries(lines as Record<string, unknown>)) {
    if (!/^(?:0x)?[0-9a-f]{16}$/i.test(lineId))
      throw new LocalisationPatchError(`${source}: invalid line ID "${lineId}"`)
    if (!languagesValue || typeof languagesValue !== 'object' || Array.isArray(languagesValue))
      throw new LocalisationPatchError(
        `${source}: line ${lineId} must contain language text values`
      )
    for (const [language, text] of Object.entries(languagesValue as Record<string, unknown>)) {
      if (!LANGUAGES.has(language))
        throw new LocalisationPatchError(`${source}: unsupported language "${language}"`)
      if (typeof text !== 'string')
        throw new LocalisationPatchError(`${source}: ${lineId}.${language} must be text`)
      target[language] ??= {}
...
export function mergeLocalisationOverrides(
  target: NonNullable<ManifestOptionData['localisationOverrides']>,
  addition: NonNullable<ManifestOptionData['localisationOverrides']>
): void {
  const mutableTarget = target as unknown as Record<string, Record<string, Record<string, string>>>
  const mutableAddition = addition as unknown as Record<
    string,
