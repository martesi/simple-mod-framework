...
import { GAME_PLATFORMS, type GamePlatform, isGamePlatform } from '../../../../shared/game'
  ...
  return (
    <div>
      ...
      <Select
        items={labels}
        value={value ?? ''}
        onValueChange={(platform) => isGamePlatform(platform) && onChange(platform)}
        disabled={disabled}
      >
      ...
      <div className="mt-1.5 text-[12px] text-text-3">
        {required ? (
          <Trans>Choose the storefront before deploying.</Trans>
        ) : value ? (
          <Trans>This storefront will be used when checking the game install.</Trans>
        ) : (
          <Trans>Choose a game folder first.</Trans>
        )}
      </div>
    </div>
  )
