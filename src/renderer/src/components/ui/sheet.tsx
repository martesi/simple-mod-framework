...
>(({ className, children, ...props }, ref) => (
  <DialogPrimitive.Portal>
    ...
    <DialogPrimitive.Popup
      ref={ref}
      className={cn(
        'fixed right-0 top-0 z-91 flex h-full w-[400px] max-w-[90vw] flex-col border-l border-border bg-surface shadow-md',
        className
      )}
      {...props}
    >
...
export function SheetHeader({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) {
  return (
    <div
      className={cn(
        'flex items-start justify-between border-b border-border px-[22px] py-5',
        className
      )}
      {...props}
    />
...
export function SheetTitle({
  ...
  return (
    <DialogPrimitive.Title
      className={cn('text-[16px] font-bold text-text', className)}
      {...props}
    />
  )
}
