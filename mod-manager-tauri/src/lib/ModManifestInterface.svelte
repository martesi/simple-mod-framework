<script lang="ts">
	...
	function toStringArr(arr: unknown): string[] { return (arr as string[]) || [] }

	let compatibility = true
	let supportedPlatforms = true
		...
		<div class="grid grid-cols-2 gap-4">
			<div>
				<TextInput
					...
					on:input={() => { contentFolderInputChanged = !!contentFolderInput.value.length }}
				/>
				<br />
				{#if source.contentFolders}
					<Button kind="ghost" icon={CloseOutline} on:click={() => dispatch("contentFolder-undefine")}>
						Remove <span class="mr-2" />
					</Button>
				{/if}
			...
			<div>
				<TextInput
					...
					on:input={() => { blobsFolderInputChanged = !!blobsFolderInput.value.length }}
				/>
				<br />
				{#if source.blobsFolders}
					<Button kind="ghost" icon={CloseOutline} on:click={() => dispatch("blobsFolder-undefine")}>
						Remove <span class="mr-2" />
					</Button>
				{/if}
		...
		{#if modLocalisation}
			<div transition:slide>
				<LocalisationEditor
					...
					on:define={({ detail }) => dispatch("localisationValue-define", detail)}
					on:undefine={({ detail }) => dispatch("localisationValue-undefine", detail)}
				/>
			</div>
		{/if}
		<br />
		...
		{#if localisationOverrides}
			<div transition:slide>
				{#each Object.entries(source.localisationOverrides || {}) as [hash, locData] (hash)}
					...
					<LocalisationEditor
						source={locData}
						on:define={({ detail }) => dispatch("localisationOverrideValue-define", { ...detail, hash })}
						on:undefine={({ detail }) => dispatch("localisationOverrideValue-undefine", { ...detail, hash })}
					/>
					<br />
				{/each}
				<br />
				<Button kind="primary" icon={Edit} on:click={() => (localisationOverridesNewFileModalOpen = true)}>
		...
		{#if localisedLines}
			<div transition:slide>
				<KeyValueEditor
					...
					on:define={({ detail }) => dispatch("localisedLine-define", detail)}
					on:undefine={({ detail }) => dispatch("localisedLine-undefine", detail)}
				/>
			</div>
		{/if}
	</div>
	...
	<div transition:slide>
							...
							<tr class:border-b={index != (source.packagedefinition || []).filter((a) => a.type == "partition").length - 1} class="border-solid border-b-black">
								<td class="p-4 pl-8 text-slate-800">
									<div class="flex flex-row gap-4 items-center">
										...
										<Button kind="ghost" size="small" icon={Edit} iconDescription="Edit name"
											on:click={() => { pdefValueToEdit = name + "$:$" + "name"; pdefValueToEditPlaceholder = name; pdefEditModalOpen = true }}
										/>
									</div>
								</td>
								<td class="p-4 px-8 text-slate-800">
									<div class="flex flex-row gap-4 items-center">
										...
										<Button kind="ghost" size="small" icon={Edit} iconDescription="Edit parent"
											on:click={() => { pdefValueToEdit = name + "$:$" + "parent"; pdefValueToEditPlaceholder = parent; pdefEditModalOpen = true }}
										/>
									</div>
								</td>
								<td class="p-4 pr-8 text-slate-800">
									<div class="flex flex-row gap-4 items-center">
										...
										<Button kind="ghost" size="small" icon={Edit} iconDescription="Edit type"
											on:click={() => { pdefValueToEdit = name + "$:$" + "partitionType"; pdefValueToEditPlaceholder = partitionType; pdefEditModalOpen = true }}
										/>
										<Button kind="ghost" size="small" icon={CloseOutline} iconDescription="Remove partition"
											on:click={() => dispatch("pdefPartition-undefine", { partition: name })}
										/>
									</div>
								</td>
							</tr>
							...
							<tr class:border-b={index != (source.packagedefinition || []).filter((a) => a.type == "entity").length - 1} class="border-solid border-b-black">
								<td class="p-4 pl-8 text-slate-800">
									<div class="flex flex-row gap-4 items-center">
										...
										<Button kind="ghost" size="small" icon={Edit} iconDescription="Edit value"
											on:click={() => { pdefValueToEdit = partition + "|" + path + "$:$" + "partition"; pdefValueToEditPlaceholder = partition; pdefEditModalOpen = true }}
										/>
									</div>
								</td>
								<td class="p-4 px-8 text-slate-800">
									<div class="flex flex-row gap-4 items-center">
										...
										<Button kind="ghost" size="small" icon={Edit} iconDescription="Edit value"
											on:click={() => { pdefValueToEdit = partition + "|" + path + "$:$" + "path"; pdefValueToEditPlaceholder = path; pdefEditModalOpen = true }}
										/>
										<Button kind="ghost" size="small" icon={CloseOutline} iconDescription="Remove entity"
											on:click={() => dispatch("pdefEntity-undefine", { entity: partition + "|" + path })}
										/>
									</div>
								</td>
							</tr>
		...
		{#if dependencies}
			<div transition:slide>
				...
				<table class="table-auto border-collapse bg-slate-200">
					...
					<tbody>
						{#each (source.dependencies || []).map((a) => typeof a == "string" ? { runtimeID: a, toChunk: 0, portFromChunk1: false } : a) as { runtimeID, toChunk, portFromChunk1 }, index (runtimeID + toChunk)}
							<tr class:border-b={index != (source.dependencies || []).length - 1} class="border-solid border-b-black">
								<td class="p-4 pl-8 text-slate-800">
									<div class="flex flex-row gap-4 items-center">
										...
										<Button kind="ghost" size="small" icon={Edit} iconDescription="Edit value"
											on:click={() => { dependenciesValueToEdit = { runtimeID, toChunk: toChunk || 0, portFromChunk1: portFromChunk1 || false, valueToEdit: "runtimeID" }; dependenciesValueToEditPlaceholder = runtimeID; dependenciesEditModalOpen = true }}
										/>
									</div>
								</td>
								<td class="p-4 pl-8 text-slate-800">
									<div class="flex flex-row gap-4 items-center">
										...
										<Button kind="ghost" size="small" icon={Edit} iconDescription="Edit value"
											on:click={() => { dependenciesValueToEdit = { runtimeID, toChunk: toChunk || 0, portFromChunk1: portFromChunk1 || false, valueToEdit: "toChunk" }; dependenciesValueToEditPlaceholder = String(toChunk || 0); dependenciesEditModalOpen = true }}
										/>
									</div>
								</td>
								<td class="p-4 px-8 text-slate-800">
									<div class="flex flex-row gap-4 items-center">
										...
										<Button kind="ghost" size="small" icon={Edit} iconDescription="Edit value"
											on:click={() => { dependenciesValueToEdit = { runtimeID, toChunk: toChunk || 0, portFromChunk1: portFromChunk1 || false, valueToEdit: "portFromChunk1" }; dependenciesValueToEditPlaceholder = String(portFromChunk1 || false); dependenciesEditModalOpen = true }}
										/>
										<Button kind="ghost" size="small" icon={CloseOutline} iconDescription="Remove dependency"
											on:click={() => dispatch("dependency-undefine", { runtimeID, toChunk, portFromChunk1 })}
										/>
									</div>
								</td>
							</tr>
		...
		{#if thumbs}
			<div transition:slide>
				<ListEditor
					data={source.thumbs || []}
					on:define={({ detail }) => dispatch("thumbs-define", detail)}
					on:undefine={({ detail }) => dispatch("thumbs-undefine", detail)}
				/>
			</div>
		{/if}
	</div>
...
{#if compatibility}
	<div transition:slide>
		...
		{#if supportedPlatforms}
			<div transition:slide>
				{#each Object.keys(Platform).filter((a) => typeof a == "string") as platform (platform)}
					<Checkbox
						checked={(source.supportedPlatforms || []).length == 0 || (source.supportedPlatforms || []).some(p => p === platform)}
						labelText={platform.slice(0, 1).toUpperCase() + platform.slice(1)}
						on:check={({ detail }) => dispatch("supportedPlatforms-alter", { platform, value: detail })}
					/>
				{/each}
			</div>
		{/if}
		...
		{#if requirements}
			<div transition:slide>
				<ListEditor
					data={toStringArr(source.requirements)}
					on:define={({ detail }) => dispatch("requirements-define", detail)}
					on:undefine={({ detail }) => dispatch("requirements-undefine", detail)}
				/>
			</div>
		{/if}
		<br />
		...
		{#if loadBefore}
			<div transition:slide>
				<ListEditor
					data={toStringArr(source.loadBefore)}
					on:define={({ detail }) => dispatch("loadBefore-define", detail)}
					on:undefine={({ detail }) => dispatch("loadBefore-undefine", detail)}
				/>
			</div>
		{/if}
		<br />
		...
		{#if loadAfter}
			<div transition:slide>
				<ListEditor
					data={toStringArr(source.loadAfter)}
					on:define={({ detail }) => dispatch("loadAfter-define", detail)}
					on:undefine={({ detail }) => dispatch("loadAfter-undefine", detail)}
				/>
			</div>
		{/if}
	</div>
...
<TextInputModal
	...
	modalInitialText=""
	on:close={() => {
		if (localisationOverridesNewFileModal.value?.length) {
...
<TextInputModal
	...
	on:close={() => {
		if (dependenciesEditModal.value?.length) {
			if (dependenciesValueToEdit.valueToEdit == "runtimeID") {
				dispatch("dependency-define", { type: "defineRuntimeID", origToChunk: dependenciesValueToEdit.toChunk, origRuntimeID: dependenciesValueToEdit.runtimeID, origPortFromChunk1: dependenciesValueToEdit.portFromChunk1, newRuntimeID: dependenciesEditModal.value })
			} else if (dependenciesValueToEdit.valueToEdit == "toChunk") {
				dispatch("dependency-define", { type: "defineToChunk", origToChunk: dependenciesValueToEdit.toChunk, origRuntimeID: dependenciesValueToEdit.runtimeID, origPortFromChunk1: dependenciesValueToEdit.portFromChunk1, newToChunk: Number(dependenciesEditModal.value) })
			} else if (dependenciesValueToEdit.valueToEdit == "portFromChunk1") {
				dispatch("dependency-define", { type: "definePortFromChunk1", origToChunk: dependenciesValueToEdit.toChunk, origRuntimeID: dependenciesValueToEdit.runtimeID, origPortFromChunk1: dependenciesValueToEdit.portFromChunk1, newPortFromChunk1: dependenciesEditModal.value == "true" })
			}
		}
	}}
/>
