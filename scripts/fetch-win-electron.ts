...
async function main(): Promise<void> {
  ...
  if (!version)
    throw new Error(
      `Could not determine the installed Electron version from ${electronPackageJson}`
    )

  const [markerExists, executableExists] = await Promise.all([
    requireTask(pathExists(versionMarker)),
    requireTask(pathExists(electronExecutable)),
  ])
