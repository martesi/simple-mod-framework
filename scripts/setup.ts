...
async function runScript(scriptName: string): Promise<void> {
  await requireTask(
    runProcess(process.execPath, [join(scriptDirectory, scriptName)], { stdin: 'inherit' })
  )
}
...
async function main(): Promise<void> {
  const alreadyFetched = (
    await Promise.all(
      setupArtifacts.map((artifact) => requireTask(pathExists(join(thirdPartyDirectory, artifact))))
    )
  ).every(Boolean)
  if (process.argv.includes('--postinstall') && alreadyFetched) {
    console.log(
      "extra/Third-Party/ already looks populated, skipping postinstall setup. Run 'bun run setup' to force."
    )
    return
  }
    ...
    (async () => {
      if (await pathExists(electronInstallScript)) {
        await runStep(
          ...
          () =>
            requireTask(
              runProcess(process.execPath, [electronInstallScript], { stdin: 'inherit' })
            ),
          'Re-run `bun run setup` later, or run `bun node_modules/electron/install.js` by hand.'
        )
      } else {
        console.warn(
          '"electron" not found in node_modules - skipping its binary download. Run `bun install` first.'
        )
      }
    })(),
    runStep(
...
try {
  ...
  console.warn(
    `Setup encountered an unexpected failure (${errorMessage(error)}). Re-run \`bun run setup\` later.`
  )
}
