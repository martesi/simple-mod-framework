...
// binary, then the release-backed tools, then hitman-hashes in that order.
...
async function runScript(scriptName: string): Promise<void> {
	await runProcess(process.execPath, [join(scriptDirectory, scriptName)], { stdin: "inherit" })
}
...
async function main(): Promise<void> {
	...
	const alreadyFetched = (await Promise.all(setupArtifacts.map((artifact) => pathExists(join(thirdPartyDirectory, artifact))))).every(Boolean)
