...
import { type ScriptError, scriptError, tryScript } from './effects'
...
export function extractArchive(
  ...
  return pipe(
    ...
    TE.chain(() => {
      if (format === 'sevenZip')
        return sevenZip
          ? runProcess(sevenZip, ['x', archivePath, `-o${destinationDirectory}`, '-y'])
          : TE.left(
              scriptError(
                'extract archive',
                'A 7-Zip executable is required to extract this archive',
                { path: archivePath }
              )
            )
            ...
            TE.chain((native) =>
              native
                ...
                : TE.left(
                    scriptError(
                      'extract archive',
                      'no "unzip", "7zz", "7z", or "7za" found on PATH to extract an upstream release archive - install one or run this from `nix develop .#e2e`',
                      { path: archivePath }
                    )
                  )
            )
...
export const extractSevenZipArchive = (
  archivePath: string,
  destinationDirectory: string,
  sevenZip: string
) => extractArchive(archivePath, destinationDirectory, 'sevenZip', sevenZip)

export function ensureArchiveFiles(
  specification: ArchiveSpecification,
  destinationDirectory: string,
  options: FetchOptions = {}
): TE.TaskEither<ScriptError, EnsureResult> {
  const destinations = specification.files.map((file) =>
    join(destinationDirectory, file.destinationName ?? file.sourceName)
  )
  const stageMember = (
    temporaryDirectory: string,
    extractionDirectory: string,
    file: ArchiveFileSpecification
  ) =>
    pipe(
      ...
      TE.chain((found) =>
        found
          ? pipe(
              tryScript(
                'stage archive member',
                () =>
                  copyFile(
                    found,
                    join(temporaryDirectory, file.destinationName ?? file.sourceName)
                  ),
                { path: found }
              ),
              TE.map(() => join(temporaryDirectory, file.destinationName ?? file.sourceName))
            )
          : TE.left(
              scriptError(
                'stage archive member',
                specification.missingFileMessage(file.sourceName),
  ...
  return pipe(
    ...
    TE.chain((present) =>
      present.every(Boolean)
        ? TE.right<ScriptError, EnsureResult>('already downloaded')
        : withTemporaryDirectory('smf-archive', (temporaryDirectory) => {
            ...
            return pipe(
              ...
              TE.chain(() =>
                extractArchive(archivePath, extractionDirectory, specification.format)
              ),
              TE.chain(() => ensureDirectory(stagedDirectory)),
              TE.chain(() =>
                TE.sequenceArray(
                  specification.files.map((file) =>
                    stageMember(stagedDirectory, extractionDirectory, file)
                  )
                )
              ),
              TE.chain((stagedFiles) =>
                TE.sequenceArray(
                  stagedFiles.map((stagedFile, index) =>
                    copyFileAtomically(stagedFile, destinations[index])
                  )
                )
              ),
