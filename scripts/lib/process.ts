...
import { type ScriptError, scriptError } from './effects'
    ...
    async () => {
      ...
      if (exitCode !== 0)
        throw scriptError(
          ...
          { process: { command, args, exitCode } }
        )
    },
    (cause) =>
      cause && typeof cause === 'object' && '_tag' in cause
        ? (cause as ScriptError)
        : scriptError('run process', String(cause), { cause, process: { command, args } })
  )
}
...
export function findExecutable(
  ...
  return TE.rightTask(async () => {
    for (const candidate of candidates) {
      ...
      const executable =
        candidate.includes('/') || candidate.includes('\\')
          ? existing?._tag === 'Right' && existing.right
            ? candidate
            : null
          : Bun.which(candidate)
      if (!executable) continue
      if (probeArgs.length === 0) return executable
      const result = await runProcess(executable, probeArgs, {
        stdin: 'ignore',
        stdout: 'ignore',
        stderr: 'ignore',
      })()
