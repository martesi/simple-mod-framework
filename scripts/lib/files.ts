...
export function removePath(filePath: string): TE.TaskEither<ScriptError, void> {
  return tryScript('remove path', () => rm(filePath, { recursive: true, force: true }), {
    path: filePath,
  })
}
...
export function createTemporaryPath(
  ...
  return pipe(
    ...
    TE.map(() => join(directory, `.${prefix}-${randomUUID()}.tmp`))
  )
}

export function moveFileAtomically(
  source: string,
  destination: string
...
export function copyFileAtomically(
  ...
  return TE.bracket(
    ...
    (temporaryPath) =>
      pipe(
        tryScript('copy file', () => copyFile(source, temporaryPath), { path: source }),
...
export function writeFileAtomically(
  ...
  return TE.bracket(
    ...
    (temporaryPath) =>
      pipe(
        tryScript('write file', () => writeFile(temporaryPath, contents), { path: destination }),
        TE.chain(() => moveFileAtomically(temporaryPath, destination))
      ),
    (temporaryPath) => cleanupTemporaryPath(temporaryPath)
  )
...
export function findFile(
  directory: string,
  name: string
): TE.TaskEither<ScriptError, string | null> {
  return tryScript(
    'find file',
    async () => {
      ...
      for (const entry of entries) {
        ...
        if (entry.isDirectory()) {
          ...
          if (found._tag === 'Right' && found.right) return found.right
