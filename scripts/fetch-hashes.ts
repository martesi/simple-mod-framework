...
const archiveUrl =
  'https://github.com/glacier-modding/hitman-hashes/releases/latest/download/latest-hashes.7z'
...
function workflow() {
  return requireTask(ensureDirectory(destinationDirectory)).then(async () => {
    ...
    if (!sevenZip || (process.platform === 'win32' && !(await requireTask(pathExists(sevenZip)))))
      throw new Error(
        'no "7zz", "7z", or "7za" found on PATH - run this from `nix develop .#e2e` or install a native 7-Zip CLI'
      )

    console.log('Fetching hitman-hashes...')
    await requireTask(
      withTemporaryDirectory('smf-hashes', (temporaryDirectory) => {
        const archivePath = join(temporaryDirectory, 'latest-hashes.7z')
        const extractionDirectory = join(temporaryDirectory, 'extracted')
        return requireTask(downloadFile(archiveUrl, archivePath, requestOptions))
          .then(() =>
            requireTask(extractSevenZipArchive(archivePath, extractionDirectory, sevenZip))
          )
          .then(async () => {
            ...
            return requireTask(
              copyFileAtomically(found, join(destinationDirectory, 'hash_list.txt'))
            )
          })
      })
    )
