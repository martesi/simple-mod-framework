...
import {
  type ArchiveSpecification,
  type EnsureResult,
  ensureArchiveFiles,
  extractSevenZipArchive,
} from './lib/archive'
import { downloadFile, type FetchOptions, fetchJson } from './lib/download'
...
const rpkgToolsArchive: ArchiveSpecification = {
  ...
  archiveUrl:
    'https://github.com/glacier-modding/RPKG-Tool/releases/download/v2.34.0/rpkg_v2.34.0-cli.zip',
  format: 'zip',
  files: [
...
const zhmToolsArchive: ArchiveSpecification = {
  ...
  archiveUrl:
    'https://github.com/OrfeasZ/ZHMTools/releases/download/v4.1.0/ResourceTool-win-x64.zip',
  format: 'zip',
  files: [
...
async function ensureQuickEntityRs(): Promise<EnsureResult> {
  ...
  await requireTask(
    downloadFile(
      'https://github.com/atampy25/quickentity-rs/releases/latest/download/quickentity-rs.exe',
      target,
      downloadOptions
    )
  )
...
async function ensureQuickEntity3(): Promise<EnsureResult> {
  ...
  await requireTask(
    downloadFile(
      'https://github.com/atampy25/quickentity-rs/releases/download/3.0/quickentity-rs.exe',
      target,
      downloadOptions
    )
  )
...
async function ensureSevenZip(): Promise<EnsureResult> {
  ...
  return requireTask(
    withTemporaryDirectory('smf-7z', (temporaryDirectory) =>
      tryScript(
        'ensure 7-Zip',
        async () => {
          ...
          if (process.platform !== 'win32' && !nativeSevenZip) {
            throw new Error(
              'no "7zz"/"7z"/"7za" found on PATH to extract the upstream 7-Zip release - run this from `nix develop .#e2e` (provides 7zz) or install p7zip'
            )
          }
          ...
          const bootstrapDownload =
            process.platform === 'win32'
              ...
              : Promise.resolve()
          const releasePromise = requireTask(
          ...
          if (debug)
            debugLog(
              `ip7z/7zip latest release ${release.tag_name}, assets: ${assetNames.join(', ')}`
            )
          ...
          await Promise.all([
            bootstrapDownload,
            requireTask(downloadFile(asset.browser_download_url, archivePath, downloadOptions)),
          ])
          ...
          if (debug)
            debugLog(
              `extracting with ${process.platform === 'win32' ? `bootstrap ${bootstrapPath}` : `native ${extractor}`}`
            )
          ...
          try {
            found = await requireTask(
              findFile(
                process.platform === 'win32'
                  ? extractionDirectory
                  : join(extractionDirectory, 'x64'),
                '7za.exe'
              )
            )
          } catch {
          ...
          if (!found) {
            throw new Error(
              `Couldn't find 7za.exe inside ${asset.name} - the package layout may have changed. Place a 7-Zip build at "extra/Third-Party/7z.exe" yourself.`
            )
          }
  ...
  ).then((result) => result as EnsureResult)
}
...
async function main(): Promise<void> {
  ...
  await Promise.all([
    task(
      'extra/Third-Party/quickentity-3.exe',
      'Place it in "extra/Third-Party/" by hand.',
      ensureQuickEntity3
    ),
    task(
      'extra/Third-Party/quickentity-rs.exe',
      'Place it in "extra/Third-Party/" by hand.',
      ensureQuickEntityRs
    ),
    task(
    ...
    task(
      'extra/Third-Party/xdelta3.exe',
      'Place xdelta3.exe in "extra/Third-Party/" by hand.',
      () => ensureArchive(xdeltaArchive)
    ),
    task(
      'extra/Third-Party/7z.exe',
      'Place a 7-Zip build at "extra/Third-Party/7z.exe" by hand.',
      ensureSevenZip
    ),
  ])
}
